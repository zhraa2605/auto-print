...
class ServerPrintService {
  ...
  async printOrderPDF(order: Order): Promise<PrinterStatus> {
    try {
      ...
      if (!chromeAvailable) {
        console.log("[v0] Chrome not available, using text-based printing")
        return this.printOrderText(order)
      }
      ...
      if (error.message.includes("Chrome") || error.message.includes("Chromium")) {
        console.log("[v0] Chrome error detected, falling back to text printing")
        return this.printOrderText(order)
      }
  ...
  private async checkChromeAvailability(): Promise<boolean> {
    try {
      ...
      console.log("[v0] Chrome not available:", error.message)
      return false
    }
  }

  private async printOrderText(order: Order): Promise<PrinterStatus> {
    try {
      console.log("[v0] Using text-based printing for order:", order.id)
      ...
      // Print text file using system command
      const printStatus = await this.printTextFile(textPath)

      // Clean up text file after printing
      setTimeout(() => {
      ...
      console.log("[v0] Order printed via text with status:", printStatus)
      return printStatus
    } catch (error) {
      console.error("[v0] Text printing failed:", error)
      return {
        success: false,
        error: error.message,
      }
  ...
  private generateOrderText(order: Order): string {
...
`
  ...
  private async printTextFile(textPath: string): Promise<PrinterStatus> {
    const { exec } = require("child_process")
    const util = require("util")
    const execPromise = util.promisify(exec)
    ...
    try {
      ...
      if (process.platform === "win32") {
        printCommand = `type "${textPath}" > PRN`
      } else if (process.platform === "darwin") {
        if (printers.length > 0) {
          printCommand = `lpr -P "${printers[0]}" "${textPath}" && echo "SUCCESS:$(lpq | tail -1 | awk '{print $1}')"`
        } else {
          printCommand = `lpr "${textPath}" && echo "SUCCESS:$(lpq | tail -1 | awk '{print $1}')"`
        }
      } else {
        printCommand = `lp "${textPath}" | awk '{print "SUCCESS:" $4}'`
      }

      const { stdout, stderr } = await execPromise(printCommand)

      if (stdout.includes("SUCCESS") || process.platform === "win32") {
        const jobId = stdout.split(":")[1]?.trim() || "text-print"
        console.log("[v0] Text file sent to printer successfully, Job ID:", jobId)
        return {
          success: true,
          printerName,
          jobId,
      ...
      console.error("[v0] Text print command failed:", error)
      return {
        success: false,
        error: error.message,
      }
